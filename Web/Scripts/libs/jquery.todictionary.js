...
(function ($)
{
	...
	var _flatten = function (input, output, prefix, includeNulls, encode)
	{
		if ($.isPlainObject(input))
		{
			for (var p in input)
			{
				...
				{
					_flatten(input[p], output, prefix.length > 0 ? prefix + "." + p : p, includeNulls, encode);
				}
			}
		}
		else
		{
			if ($.isArray(input))
			{
				...
				{
					_flatten(value, output, "{0}[{1}]".format(prefix, index), encode);
				});
				return;
			}
			if (!$.isFunction(input))
			{
				...
				{
					output.push({ name: prefix, value: encode ? encodeURIComponent(input.toISOString()) : input.toISOString() });
				}
				else
				{
					...
					output.push({ name: prefix, value: encode ? encodeURIComponent(val) : val });
				}
			}
		}
	};
	...
	$.extend({
		toDictionary: function (data, prefix, includeNulls, encode)
		{
			...
			var result = [];
			_flatten(data, result, prefix || "", includeNulls, encode);
